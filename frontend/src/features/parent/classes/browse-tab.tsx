// frontend/src/features/parent/classes/browse-tab.tsx
//
    ...
    >
      {list => (
        <div className="flex flex-col gap-6">
          {list.length > 1 ? (
            <ChildPicker students={list} value={studentId} onChange={setStudentId} />
          ) : (
            child && (
              <p className="text-sm text-muted-foreground">
                Classes for {child.first_name}{child.level_name ? `, ${child.level_name}` : ''}, and classes open to every level.
              </p>
            )
          )}
